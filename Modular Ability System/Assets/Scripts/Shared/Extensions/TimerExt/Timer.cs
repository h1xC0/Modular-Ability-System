...
/// </summary>
namespace Shared.Extensions.TimerExt {
public class Timer
{
    ...
    public static Timer Register(float startValue, float duration, Action onComplete, Action<float> onUpdate = null,
        bool isLooped = false, bool useRealTime = false, MonoBehaviour autoDestroyOwner = null)
    {
        // create a manager object to update all the timers if one does not already exist.
        if (Timer._manager == null)
        ...
        Timer timer = new Timer(startValue, duration, onComplete, onUpdate, isLooped, useRealTime, autoDestroyOwner);
        Timer._manager.RegisterTimer(timer);
        return timer;
    }
    ...
    private Timer(float startValue, float duration, Action onComplete, Action<float> onUpdate,
        bool isLooped, bool usesRealTime, MonoBehaviour autoDestroyOwner)
    {
        this.duration = duration;
        this._onComplete = onComplete;
        ...
        this._startTime = this.GetWorldTime();
        this._lastUpdateTime = this._startTime;
        this._startTimeOffset = startValue;
    }
    ...
    {
        return this.usesRealTime ? Time.realtimeSinceStartup : Time.time + _startTimeOffset;
    }
