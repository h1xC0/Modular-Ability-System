...
namespace Shared.Extensions
{
    public class VisualizeBox
    {
        ...
        static Vector3[]
            RotateObject(Vector3[] ObjToRotate, Vector3 DegreesToRotate,
                Vector3 Around) //rotates a set of dots counterclockwise
        {
            ...
            for (int i = 0; i < ObjToRotate.Length; i++)
            {
                ...
                if (H != 0)
                {
                    ...
                    ObjToRotate[i] = new Vector3(H * (CosA * cosB - SinA * SinB), H * (SinA * cosB + CosA * SinB),
                        ObjToRotate[i].z);
                }
            }
            ...
            for (int i = 0; i < ObjToRotate.Length; i++)
            {
                ...
                if (H != 0)
                {
                    ...
                    ObjToRotate[i] = new Vector3(ObjToRotate[i].x, H * (CosA * cosB - SinA * SinB),
                        H * (SinA * cosB + CosA * SinB));
                }
            }
            ...
            for (int i = 0; i < ObjToRotate.Length; i++)
            {
                ...
                if (H != 0)
                {
                    ...
                    ObjToRotate[i] = new Vector3((CosA * cosB - SinA * SinB) * H, ObjToRotate[i].y,
                        H * (SinA * cosB + CosA * SinB));
                }
            }
